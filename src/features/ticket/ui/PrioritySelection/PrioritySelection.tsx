...
export const PrioritySelection: React.FC = () => {
    ...
    const handlePrioritySelect = async (priority: typeof priorities[0]) => {
        console.log("PrioritySelection: Starting ticket generation...", { serviceId, agenceId, priority: priority.id });

        if (!serviceId || !agenceId || isGenerating) {
            console.warn("PrioritySelection: Aborting - missing data or already generating", { serviceId, agenceId, isGenerating });
            return;
        }

        setIsGenerating(true);
        try {
            // 1. (Guichet selection deferred to call time)

            // 2. Générer le numéro de ticket (Préfixe + nombre de tickets du jour + 1)
            ...
            if (countError) console.error("PrioritySelection: Count error:", countError);
            ...
            // 4. Naviguer vers Done avec les infos
            navigate(`${basePath}/done`, {
                state: {
                    ticket: newTicket,
                    serviceName: serviceName,
                    priorityName: priority.name,
                    agenceName: agenceName  // Transmis au microservice d'impression
                }
            });
        } catch (err: any) {
            console.error("PrioritySelection: Global error handler:", err);
            alert(`Erreur lors de la génération du ticket: ${err.message || "Erreur inconnue"}`);
        } finally {
        ...
        return (
            <div className="kiosk-container priority-selection-page">
                <main className="kiosk-main">
                    ...
                    <button className="kiosk-back-button" onClick={() => navigate(basePath)}>
                        Retour
                    </button>
                </main>
            </div>
    ...
    return (
        <div className="kiosk-container priority-selection-page">
            ...
            <main className="kiosk-main">
                ...
                <h2 className="kiosk-page-title">Choisissez votre niveau pour {serviceName}</h2>
                    ...
                    {priorities.map((priority) => (
                        ...
                        >
                            <div className="priority-card-image-wrapper">
                                <img src={priority.image} alt={priority.name} className="priority-image" />
                            </div>
                ...
                <button className="kiosk-back-button" onClick={() => navigate(basePath)} disabled={isGenerating}>
