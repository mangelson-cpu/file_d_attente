...
export const AgenceManager: React.FC = () => {
  ...
  const { itemsPerPage, needsPagination } = useDynamicPageSize(tableContainerRef, agences.length);
      ...
      <div className="content-card" ref={tableContainerRef}>
                ...
                <tr key={agence.id}>
                  ...
                  <td>
                    {agence.slug ? (
                      ...
                      <span className="text-secondary text-sm">Non généré</span>
                    )}
                  </td>
                  <td>
                    {agence.slug ? (
        ...
        {needsPagination && (
          <div className="pagination-controls">
            ...
            {Array.from({ length: Math.ceil(agences.length / itemsPerPage) }, (_, i) => i + 1).map(page => (
              <button key={page} className={`pagination-btn ${currentPage === page ? 'active' : ''}`} onClick={() => setCurrentPage(page)}>{page}</button>
            ))}
            <button className="pagination-btn" disabled={currentPage === Math.ceil(agences.length / itemsPerPage)} onClick={() => setCurrentPage(p => p + 1)}>→</button>
            <span className="pagination-info">{agences.length} agence{agences.length > 1 ? 's' : ''}</span>
          </div>
        )}
      </div>
    </div>
