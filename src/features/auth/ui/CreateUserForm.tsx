...
export const CreateUserForm: React.FC<Props> = ({ agences, userRole, currentUserAgenceId }) => {
  const [users, setUsers] = useState<User[]>([]);
  const [showModal, setShowModal] = useState(false);
  const [currentPage, setCurrentPage] = useState(1);
  const tableContainerRef = useRef<HTMLDivElement>(null);
  const { itemsPerPage, needsPagination } = useDynamicPageSize(tableContainerRef, users.length);
  ...
  // Pour un admin, l'agence est fixée à la sienne
  useEffect(() => {
  ...
  const fetchUsers = useCallback(async (ignore: boolean = false) => {
    ...
    try {
      ...
      if (data) {
        // Obtenir l'utilisateur actuel pour l'exclure de la liste
        const { data: { user } } = await supabase.auth.getUser();
        const currentUserId = user?.id;

        const filteredUsers = (data as User[]).filter(u => u.id !== currentUserId);
      ...
      setFetchError(err.message || "Erreur réseau inconnue lors du chargement.");
      setUsers([]);
    }
  }, []);
  ...
  const resetForm = () => {
    ...
    setPassword("");
    setRole("user");
    // Ne pas reset l'agenceId si c'est un admin (il doit rester sur la sienne)
    if (userRole !== "admin") {
      setAgenceId("");
    }
    setMessage("");
  ...
  return (
    <div className="agents-page">
      <header className="page-header">
        ...
        <button className="primary-gradient-btn" onClick={() => setShowModal(true)}>
          + Créer un agent
        </button>
      </header>
        ...
        <div className="modal-overlay" onClick={() => !loading && setShowModal(false)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="auth-card-header" style={{ marginBottom: "2rem" }}>
              ...
              <p className="auth-card-subtitle">Remplissez les informations ci-dessous</p>
            </div>
            ...
            <form className="auth-form" onSubmit={handleSubmit}>
              <div className="auth-input-grid">
                <div className="auth-input-group">
                  ...
                  <input className="auth-input" type="text" value={nom} onChange={e => setNom(e.target.value)} required />
                </div>
                <div className="auth-input-group">
                ...
                <div className="auth-input-group">
                  ...
                  <select
                    ...
                    disabled={userRole === "admin"} // Un admin ne peut créer que des 'user' normalement
                  >
                    <option value="user">Utilisateur</option>
                    {userRole === "super_admin" && <option value="admin">Administrateur</option>}
                  </select>
                </div>
                <div className="auth-input-group" style={{ gridColumn: "span 2" }}>
                  ...
                  <select
                    ...
                    disabled={userRole === "admin"} // Bloqué pour l'admin
                  >
                    <option value="">
                      {userRole === "super_admin" ? "Aucune agence (Super Admin)" : "Sélectionner une agence"}
                    </option>
                    {agences.map(a => <option key={a.id} value={a.id}>{a.nom}</option>)}
                  </select>
                  {userRole === "admin" && (
                    <p style={{ fontSize: "0.75rem", color: "#64748b", marginTop: "0.4rem" }}>
                      En tant qu'administrateur, vous créez des agents pour votre propre agence.
                    </p>
                  )}
                </div>
              </div>
              ...
              {message && <div className={`auth-message ${isSuccess ? "auth-message--success" : "auth-message--error"}`} style={{ marginTop: "1rem" }}>{message}</div>}
            </form>
          </div>
        </div>
      )}
      ...
      <div className="content-card" ref={tableContainerRef}>
        {fetchError && (
          <div className="auth-message auth-message--error" style={{ marginBottom: "1rem", textAlign: "left", fontWeight: "bold" }}>
            Erreur de chargement ({new Date().toLocaleTimeString()}) : {fetchError}
          </div>
        )}
              ...
              <tr key={user.id}>
                ...
                <td>
                  <span className={`status-badge ${user.role}`}>
                    {user.role === 'super_admin' ? 'Super User' :
        ...
        {needsPagination && (
          <div className="pagination-controls">
            ...
            <button className="pagination-btn" disabled={currentPage === Math.ceil(users.length / itemsPerPage)} onClick={() => setCurrentPage(p => p + 1)}>→</button>
            <span className="pagination-info">{users.length} agent{users.length > 1 ? 's' : ''}</span>
          </div>
        )}
      </div>
    </div>
