...
export const PublicDashboardPage = ({ userAgenceId }: Props) => {
    ...
    if (slug && loading) {
        return <div style={{ display: "flex", height: "100vh", alignItems: "center", justifyContent: "center", fontSize: "1.2rem", fontWeight: "bold", background: "var(--background-color)", color: "var(--text-primary)" }}>Chargement de l'écran...</div>;
    }

    if (slug && error) {
        return <div style={{ display: "flex", height: "100vh", alignItems: "center", justifyContent: "center", color: "var(--danger-color)", fontSize: "1.2rem", fontWeight: "bold", background: "var(--background-color)" }}>{error}</div>;
    }
